...
export async function GET(request: NextRequest, context: RouteContext<"/api/v2/waybills/[waybillNo]/skus/[skuCode]">) {
  ...
  const waybill = mapRowsToWaybill(rows);
  const sku = waybill.skus.find((item) => item.skuCode === decodeURIComponent(skuCode));
  if (!sku) {
    return NextResponse.json({ success: true, requestId: rid, data: { valid: false, sku: null, waybill } });
  }

  return NextResponse.json({ success: true, requestId: rid, data: { valid: true, sku, waybill } });
}
